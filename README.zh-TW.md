...
agent-interlude 是一個**明確的 reverse proxy**：

```
Claude Code ──(A) 純 HTTP 明文──▶ agent-interlude proxy ──(B) 正常 HTTPS──▶ api.anthropic.com
...
[`uv tool`](https://docs.astral.sh/uv/concepts/tools/) 一次裝好——`agent-interlude`
...
零 runtime dependency——agent-interlude 只用 stdlib。

要 hack agent-interlude 本身，看下方「[開發環境](#開發環境)」。
...
[agent-interlude] codex:  http://127.0.0.1:8790 -> https://chatgpt.com      (Codex + ChatGPT 登入)
[agent-interlude] logging to .agent-interlude/log-<時間戳>.jsonl
```
...
然後每次用 `-c model_provider=agent-interlude` 切過去（**別**設頂層 `model_provider`，
...
log 在 `.agent-interlude/log-<時間戳>.jsonl`，每筆交換**兩行**、用 `id` 配對：
...
```bash
agent-interlude-analyze                    # 讀 .agent-interlude 全部 log
agent-interlude-analyze --agent claude     # 只看單一 agent
agent-interlude-analyze --max-slots 30     # 多印幾條動態插槽
agent-interlude-analyze path/to/log.jsonl  # 指定檔 / glob
```
...
```bash
agent-interlude-report serve                  # http://127.0.0.1:8000（預設）
...
- `.agent-interlude/` 含**完整 prompt**（程式碼、可能的機密）→ 已 gitignore，**別 commit、別分享**。
...
編 `src/agent_interlude/proxy.py` 頂端的 `LISTENERS`，加一行 `(port, upstream_host, label)`。
wire 偵測在 `detect_wire()`、欄位正規化在 `extract()`（請求）與 `reconstruct()`（回應）。

## 開發環境

要 hack agent-interlude 本身，clone repo 直接 src layout 跑：

```bash
git clone https://github.com/zondatw/agent-interlude.git
cd agent-interlude
uv sync                              # editable install
uv run agent-interlude                     # 從 src/agent_interlude/ 跑
```
...
| 路徑 | 用途 |
|---|---|
| `src/agent_interlude/proxy.py` | 三 listener reverse proxy，streaming relay + SSE tee/重組。Entry point: `agent-interlude` |
| `src/agent_interlude/analyze.py` | 跨請求 diff、固定骨幹 vs 動態插槽、跨 agent 比較（純文字報告）。Entry point: `agent-interlude-analyze` |
| `src/agent_interlude/report.py` | 本地 web UI（HTML + JSON），對應同樣的分析。Entry point: `agent-interlude-report` |
...
| `.agent-interlude/` | JSONL 輸出，落在使用者的 cwd（gitignored，敏感） |
